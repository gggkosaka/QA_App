...
class QuestionSendActivity : AppCompatActivity(), View.OnClickListener, DatabaseReference.CompletionListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == CHOOSER_REQUEST_CODE) {
            ...
            // 画像を取得
            // dataがnullかdata.dataがnullの場合はカメラで撮影したときなので画像の取得にmPictureUriを使います。
            // data.dataで取得出来た場合はそのURIを使用します。
    ...
    override fun onClick(v: View) {
        if (v === imageView) {
            ...
            genreRef.push().setValue(data, this) // 第2引数にはCompletionListenerクラスを指定します（今回はActivityがCompletionListenerクラスを実装している）
